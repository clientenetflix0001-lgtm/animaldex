...
export const config = {
  matcher: ['/p/:path*', '/pet/:path*', '/a/:path*', '/m/:path*', '/'],
};
...
export default function middleware(request) {
  ...
  const listing = url.pathname.match(/^\/m\/([^/]+)\/?$/);
  if (listing) {
    rewrite = new URL('/api/og', url.origin);
    rewrite.searchParams.set('type', 'listing');
    rewrite.searchParams.set('id', decodeURIComponent(listing[1]));
  }
